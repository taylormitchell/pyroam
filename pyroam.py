...
class Block:
    def __init__(self, content=None, children=None, uid="", create_time=0, 
                 create_email="",  edit_time=0, edit_email="", roam_db=None, 
                 parent_blocks=None, parent_page=None):
        self.content = content or BlockContent()
        self.children = children or BlockChildren()
        self.uid = uid
        self.create_time = dt.datetime.fromtimestamp(int(create_time)/1000)
        self.create_email = create_email
        self.edit_time = dt.datetime.fromtimestamp(int(edit_time)/1000)
        self.edit_email = edit_email
    ...
    def from_dict(cls, block, roam_db):
        ...
        return cls(content, children, block['uid'], block.get('create-time',0),
                   block.get('create-email',''), block.get('edit-time',0), block.get('edit-email',''), roam_db)
